...
def expand_inputs(input_path: Path) -> list[Path]:
    ...
    if not input_path.exists():
        raise FileNotFoundError(
            ...
            "--end-date 2025-03-10 --region east_coast\n"
            "  2) Point to one local file:\n"
            "     python PROCESS_noaa_long_coastal.py "
            "--input data/temp/ais-2025-03-10.csv.zst --region east_coast\n"
            "  3) Put files under data/raw/noaa_ais/ and run:\n"
            "     python PROCESS_noaa_long_coastal.py --input data/raw/noaa_ais --region east_coast"
        )
...
def discover_ais_inputs() -> list[Path]:
    patterns = (
        ...
        "data/temp/*.csv",
        "data/temp/*.zip",
        "data/processed/Eastern coast/days/**/*.parquet",
        "*.csv.zst",
        "ais-*.csv.zst",
        "aisdk-*.csv",
    )
...
def add_coastal_argument_group(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        ...
        default="east_coast",
        help="Predefined coastal region filter.",
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Process NOAA MarineCadastre or Danish AIS data for long-horizon "
            "coastal trajectory prediction."
        )
    )
