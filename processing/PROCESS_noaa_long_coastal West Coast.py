...
# Default region: US West Coast (California through Pacific Northwest).
...
def expand_inputs(input_path: Path) -> list[Path]:
    ...
    if not input_path.exists():
        raise FileNotFoundError(
            ...
            "--end-date 2025-03-10 --region west_coast\n"
            "  2) Point to one local file:\n"
            "     python \"PROCESS_noaa_long_coastal West Coast.py\" "
            "--input data/temp/ais-2025-03-10.csv.zst --region west_coast\n"
            "  3) Put files under data/raw/noaa_ais/ and run:\n"
            "     python \"PROCESS_noaa_long_coastal West Coast.py\" "
            "--input data/raw/noaa_ais --region west_coast"
        )
...
def discover_ais_inputs() -> list[Path]:
    patterns = (
        ...
        "data/temp/*.csv",
        "data/temp/*.zip",
        "data/processed/West Coast/days/**/*.parquet",
        "*.csv.zst",
        "ais-*.csv.zst",
        "aisdk-*.csv",
    )
...
def build_sequence_windows(
    ...
    *,
    ...
    Each row contains:
        X: flattened past sequence
        y: flattened future lat/lon sequence
        plus metadata fields.

    This is simple and works with parquet/sklearn.
    For PyTorch later, we can reshape:
        X -> (history_steps, n_features)
        y -> (future_steps, 2)
    """
    start = time.perf_counter()
...
def add_coastal_argument_group(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        ...
        help="West Coast coastal region filter (default: full US Pacific coast).",
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "trajectory prediction on the US West Coast."
        )
    )
