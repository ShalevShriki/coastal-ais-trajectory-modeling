...
# Ensure the project root (parent of this processing/ dir) is importable
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))

import pandas as pd
import requests

from PROCESS_noaa_long_coastal import (
    add_coastal_argument_group,
    ensure_dirs,
    finalize_coastal_pipeline,
    format_duration,
    resolve_geo_args,
...
def days_dir(region_label: str) -> Path:
    return Path("data/processed/days") / region_label


def state_file(region_label: str) -> Path:
    return Path("data/processed") / f"processed_days_{region_label}.txt"
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Download NOAA AIS day-by-day, clean by coastal region, delete raw files, "
            "and build long-horizon trajectory windows."
        )
    )
    ...
    parser.add_argument(
        "--dataset-tag",
        help="Suffix for separate dataset storage, e.g. '4d' -> east_coast_4d.",
    )
